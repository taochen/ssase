...
import jmetal.metaheuristics.msga.MNSGA2_SAS_main;
...
public class MNSGAII extends NSGAIIRegion {
	...
	protected SASAlgorithmAdaptor getAlgorithm() {
		return new MNSGA2_SAS_main() {
			...
			protected Solution findSoleSolutionAfterEvolution(SolutionSet pareto_front) {
				if (index < 0) {
					// find the knee point
					Solution individual = ((jmetal.metaheuristics.msga.MNSGAII_SAS) algorithm)
