...
public class FIMTDD extends AbstractClassifier  {
	...
	public ClassOption splitCriterionOption = new ClassOption(
			...
			SplitCriterion.class, "org.ssase.model.onoff.VarianceReductionSplitCriterion");
...
//			0.05, 0.0, 1.0);
	public double tieThresholdOption = 0.005;
//
...
//			0.005, 0.0, 1.0);
	public double PageHinckleyAlphaOption = 0.005;
//
...
//			50, 0, Integer.MAX_VALUE);
	public int PageHinckleyThresholdOption = 50;
//
...
//			0.995, 0.0, 1.0);
	public double alternateTreeFadingFactorOption = 0.995;
//
...
//			150, 0, Integer.MAX_VALUE);
	...
	public int alternateTreeTMinOption = 150;
//
...
//			0.02, 0, 1.00);
	...
	public double learningRatioOption = 0.02;
//
...
//			0.001, 0, 1.00);
	...
	public double learningRateDecayFactorOption = 0.001;
//
...
//			"learningRatioConst", 'p', "Keep learning rate constant instead of decaying.");
	...
	public boolean learningRatioConstOption = false;
	...
	public static class LeafNode extends Node {
		...
		public void checkForSplit(FIMTDD tree) {
			...
			if (examplesSeen - examplesSeenAtLastSplitEvaluation >= tree.gracePeriodOption) {
	...
	public class FIMTDDPerceptron implements Serializable {
		...
		public void updatePerceptron(Instance inst) {
			...
			if (tree.learningRatioConstOption) {
				learningRatio = learningRatioOption;
			} else {
				learningRatio = learningRatioOption/ (1 + instancesSeen * tree.learningRateDecayFactorOption);
			}
			...
			if (currentNode instanceof LeafNode) {
				...
				if (!inAlternate && iNode.alternateTree != null) {
					...
					for (int i = 0; i < inst.weight(); i++) {
						iNode.lossFadedSumOriginal = lossO + alternateTreeFadingFactorOption * iNode.lossFadedSumOriginal;
						iNode.lossFadedSumAlternate = lossA + alternateTreeFadingFactorOption * iNode.lossFadedSumAlternate;
					...
					if (iNode.lossExamplesSeen - iNode.previousWeight >= alternateTreeTMinOption) {
						...
						if (Qi > 0) {
							...
							altTree = false;
								...
								(QiAverage < previousQiAverage && iNode.lossExamplesSeen >= (10 * this.gracePeriodOption))
								|| iNode.lossExamplesSeen >= alternateTreeTimeOption
								) {
				...
				if (iNode.changeDetection && !inAlternate) {
					if (iNode.PageHinckleyTest(normalError - iNode.sumOfAbsErrors / iNode.examplesSeen - PageHinckleyAlphaOption, PageHinckleyThresholdOption)) {
	...
	protected void attemptToSplit(LeafNode node, Node parent, int parentIndex) {
		...
		if (bestSplitSuggestions.length < 2) {
			...
			double hoeffdingBound = computeHoeffdingBound(1, this.splitConfidenceOption, node.examplesSeen);
			...
			if ((secondBestSuggestion.merit / bestSuggestion.merit < 1 - hoeffdingBound) || (hoeffdingBound < this.tieThresholdOption)) {
				shouldSplit = true;
			}
