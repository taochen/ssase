...
6. Build and deploy SSASE along with the adaptable software. Note that we currently do not provide sub-build for each sub-framework explicitly. Thus, to use only one sub-framework (e.g., FEMOSAA), one need to manually switch off the others in [ControlBus.java](https://github.com/taochen/ssase/blob/master/src/main/java/org/ssase/ControlBus.java).